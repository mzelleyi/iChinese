...
public class BookmarkFragment extends Fragment {
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        mBookmarkListFragment = (BookmarkListFragment) fm
                .findFragmentById(R.id.fragment_bookmark_listview);

        // Set Edit Button On Click Listener
        final Button editBtn = (Button) parentActivity
                .findViewById(R.id.bookmark_titlebar_btn_edit);
    ...
    public static class BookmarkListFragment extends ListFragment implements
        ...
        public void onListItemClick(ListView l, View v, int position, long id) {
            ...
            if (mAdapter.bEditMode) {
                ...
                if (mAdapter.getItemViewType(position) == VIEW_TYPE_NORMAL) {
                    ...
                    Log.d(TAG, "[BookmarkListFragment][onItemClick] dictionary info = "
                            + dictionary.getDestiontion());
        ...
        public void onLoadFinished(Loader<List<Dictionary>> loader, List<Dictionary> data) {
            ...
            if (originSize > 0) {
                ...
                Log.d(TAG, "add isDivider = " + firstAddEntry.getIsDivider() + "; entry = "
                        + firstAddEntry.getKeyword());
                for (int i = 0; i < originSize; i++) {
                    Log.d(TAG, " i = " + i);
                    ...
                    if (String.valueOf(thisChar).equalsIgnoreCase(temp)) {
                        Log.d(TAG,
                                "add isDivider = " + entry.getIsDivider() + "; entry = "
                                        + entry.getKeyword());
                        ...
                        Log.d(TAG, "add isDivider = " + addEntry.getIsDivider() + "; entry = "
                                + addEntry.getKeyword());
            ...
            // Log.d(TAG, "[BookmarkListFragment] sDividerChars length = "+
            // sDividerCharList.size());
                            ...
                            int visibleItemCount, int totalItemCount)
                    {
                        ...
                        if (mReady) {
                            if (mWithDividerList.size() > 0) {
                                // char firstLetter =
                                // mWithDividerList.get(firstVisibleItem).getKeyword().charAt(0);
                                mFirstLetter = mWithDividerList.get(firstVisibleItem).getKeyword()
                                        .charAt(0);
        ...
        private void leaveEditMode() {
            ...
            Log.d(TAG, "[BookmarkListFragment] After Remove Delete Item, The Size ="
                    + mWithDividerList.size());
                ...
                if (entry.getIsDivider()) {
                    ...
                    if (nextPos < mWithDividerList.size()) {
                        ...
                        if (nextEntry.getIsDivider()) {
                            ...
                            // Log.d(TAG,
                            // " now count ="+mWithDividerList.size());
                        }
                    } else {
            ...
            // Log.d(TAG,
            // "[BookmarkListFragment] After Remove Unused Divider, The sDividerCharList Size ="+sDividerCharList.size());
            Log.d(TAG,
                    "[BookmarkListFragment] After Remove Unused Divider, The mWithDividerList Size ="
                            + mWithDividerList.size());
    ...
    public static class BookmarkListLoader extends AsyncTaskLoader<List<Dictionary>> {
        ...
        public void deliverResult(List<Dictionary> BookmarkList) {
            ...
            // 'oldBookmarkList' if needed; now that the new result is delivered
            // we
    ...
    public static class BookmarkListAdapter extends ArrayAdapter<Dictionary> {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            Log.d(TAG, "[BookmarkListAdapter][getView] + pos = " + position + ",EditMode = "
                    + bEditMode);
            // View view = null;
            ...
            if (item != null) {
                if (convertView == null) {
                    if (this.getItemViewType(position) == VIEW_TYPE_DIVIDER) {
                        ...
                        convertView = mInflater.inflate(R.layout.bookmark_list_separate, parent,
                                false);
                        separatorText = (TextView) convertView
                                .findViewById(R.id.bookmark_listitem_separate_text);
                    } else {
                        ...
                        holder.listImageDelete = (ImageView) convertView
                                .findViewById(R.id.bookmark_listview_viewstub_img);
                        holder.listImageSpearker = (ImageView) convertView
                                .findViewById(R.id.bookmark_listitem_icon_speaker);
                        holder.listTextView1 = (TextView) convertView
                                .findViewById(R.id.bookmark_listitem_text_first_line);
                        holder.listTextView2 = (TextView) convertView
                                .findViewById(R.id.bookmark_listitem_text_second_line);
                        holder.listTextView3 = (TextView) convertView
                                .findViewById(R.id.bookmark_listitem_text_third_line);
                        holder.listTextCategory = (TextView) convertView
                                .findViewById(R.id.bookmark_listitem_text_type);
                    ...
                    if (this.getItemViewType(position) == VIEW_TYPE_DIVIDER) {
                        ...
                        // convertView =
                        // mInflater.inflate(R.layout.bookmark_list_separate,
                        // parent, false);
                        separatorText = (TextView) convertView
                                .findViewById(R.id.bookmark_listitem_separate_text);
                    } else {
